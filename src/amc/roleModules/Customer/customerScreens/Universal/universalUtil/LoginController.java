package amc.roleModules.Customer.customerScreens.Universal.universalUtil;

import javax.swing.*;

import amc.roleModules.Customer.customerScreens.SessionBus;
...
public class LoginController {
    ...
    private void attemptLogin() {
        ...
        try {
            ...
            if (userRecord != null && passwordInput.equals(userRecord.getPassword())) {
                ...
                // 🔹 Build a concrete User object for the session bus
                User sessionUser = new User();
                String id = coalesce(userRecord.getUsername(), userRecord.getEmail()); // use username first, fallback to email
                sessionUser.setUserId(id);
                sessionUser.setName(coalesce(userRecord.getUsername(), userRecord.getEmail(), "Customer"));
                sessionUser.setEmail(userRecord.getEmail());
                // phone/address/password may be absent in your record; set what you have:
                sessionUser.setPassword(userRecord.getPassword());
                // (You can also set role somewhere else if your User supports it.)

                // ✅ Store into SessionBus so dashboards can read the current user
                SessionBus.set(sessionUser);
    ...
    // Small helper: return the first non-null/non-blank string
    private static String coalesce(String... items) {
